							...
							<table class="table table-hover" id="pc-dt-simple">
								...
								<tbody>
									...
									<tr>
										...
										<td>
											@if($booking->status == 0)
											<a href="#" class="avtar avtar-xs btn-light-success" onclick="event.preventDefault(); document.getElementById('form-{{ $booking->id }}').submit();">
												<i class="ti ti-check f-20 text-success"></i>
											</a>
											<form action="{{ route('admin.bookings.approve', $booking->id) }}" method="POST" style="display:none;" id="form-{{$booking->id}}">
												@csrf
												@method('PUT')
											</form>
											@else
